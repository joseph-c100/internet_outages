...
async function fetchDataAndPlot() {
  try {
    ...
    const dataForLineChart = jsonData7days.result.annotations.map(
      (annotation) => ({
    ...
    const dataForCauseBarChart = jsonData3months.result.annotations.map(
      (annotation) => ({
        OutageCause: annotation.outage.outageCause,
        Country: annotation.locationsDetails[0]?.name || "",
      })
    ...
    const causeBarChart = Plot.plot({
      ...
      marks: [
        ...
        Plot.barY(
          ...
          Plot.groupX({ y: "count" }, { x: "OutageCause" })
        ),
