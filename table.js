...
async function fetchData() {
  try {
    ...
    const jsonData7days = await d3.json(
      "https://internetshutdowns.joeclark176.workers.dev/",
      d3.autoType
    );
    const jsonData3months = await d3.json(
    ...
    const tableHeader7days = d3
      .select("#outageTableWeek")
      .append("thead")
      .append("tr")
      .selectAll("th")
      .data(columnHeaders)
    ...
    const dataForTable7days = jsonData7days.result.annotations.map(
      (annotation) => ({
    ...
    rows7days
      .append("td")
      .text((d) =>
        d.endDate !== currentDate ? d.endDate : "null (Possibly ongoing)"
      );
    ...
    const tableHeader3months = d3
      .select("#outageTableMonths")
      .append("thead")
      .append("tr")
      .selectAll("th")
      .data(columnHeaders)
    ...
    const dataForTable3months = jsonData3months.result.annotations.map(
      (annotation) => ({
        startDate: annotation.startDate,
        endDate: annotation.endDate || currentDate,
        OutageCause: annotation.outage.outageCause,
